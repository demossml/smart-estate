...
import { encryptToken, decryptToken, validateApiKey, logSecurityEvent, hashApiKey } from './crypto';
...
import { getCurrentHouseMode, setCurrentHouseMode, reloadScenarios } from './engine';
import { createManualOverride, cleanExpiredOverrides } from './override-manager';
...
// Rate limiter для webhook-эндпоинта (не требует авторизации)
const webhookLimiter = rateLimit({
  windowMs: 60_000,
  max: 60,
  standardHeaders: true,
  legacyHeaders: false,
  message: { ok: false, error: 'Webhook rate limit exceeded (60/min per webhook_id)' },
  keyGenerator: (req) => req.params.webhook_id || req.ip || 'unknown',
...
// CSRF_SECRET — если не задан, защита от CSRF отключена
if (!process.env.CSRF_SECRET) {
  console.warn('⚠️ CSRF_SECRET не задан — защита от CSRF отключена. Рекомендуется задать в production.');
}

// ── Синхронизация API-ключей при старте ────────────────
try {
  const envKeyString = process.env.API_KEYS || '';
  const envKeys = envKeyString.split(',').map(k => k.trim()).filter(Boolean);
  if (envKeys.length > 0) {
    for (const key of envKeys) {
      const existing = db.prepare('SELECT key_value FROM api_keys WHERE key_value = ?').get(key);
      if (!existing) {
        db.prepare(
          "INSERT INTO api_keys (key_name, key_value) VALUES (?, ?) ON CONFLICT(key_name) DO UPDATE SET key_value = excluded.key_value"
        ).run(`env_${hashApiKey(key)}`, key);
        logger.log("[API]", `🔑 Импортирован ключ из ENV: ${hashApiKey(key)}`);
      }
    }
    logger.log("[API]", `🔑 Синхронизация ключей: ${envKeys.length} из ENV`);
  }
} catch (e: any) {
  logger.error("[API]", `⚠️ Ошибка синхронизации ключей: ${e.message}`);
}
...
app.use((req, res, next) => {
  ...
  // Пропускаем login (он сам проверяет ключ)
  ...
  try {
    ...
    const unified = [...noRoomDevices, ...z2mDevices
      .filter((d: any) => {
        ...
        // ⚡ Router'ы могут быть датчиками (Tuya TS0601) — не фильтруем если есть definition
        if (['Coordinator'].includes(d.type || '') || d.disabled) return false;
        if (d.type === 'Router' && !d.definition?.model && !d.model_id) return false;
        return !existingMap.has(ieee) && !['Coordinator'].includes(d.type || '');
      })
      .map((z2m: any) => {
...
app.post('/api/devices/:id/on', commandLimiter, async (req, res) => {
  ...
  try {
    ...
    // Создаём manual_override (design-doc раздел 6)
    createManualOverride(id, 'ON').catch(() => {});
...
app.post('/api/devices/:id/off', commandLimiter, async (req, res) => {
  ...
  try {
    ...
    // Создаём manual_override (design-doc раздел 6)
    createManualOverride(id, 'OFF').catch(() => {});
...
// ── PATCH /api/devices/:id (update: name, type, room_id) ──
app.patch('/api/devices/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { friendly_name, type, room_id, room_manually_set } = req.body;

    const existing = db.prepare(`SELECT * FROM devices WHERE ieee_addr = ?`).get(id) as any;
    if (!existing) {
      return res.status(404).json({ ok: false, error: 'Device not found' });
    }
    ...
    db.prepare(`
      ...
      WHERE ieee_addr = ?
    `).run(friendly_name ?? null, type ?? null, type ?? null, room_id ?? null, room_id !== undefined ? (room_id !== null ? 1 : 0) : null, id);

    if (type && type !== existing.type) {
      logStateChange(id, existing.type, type, 'type_changed');
    }

    res.json({ ok: true, updated: id });
  } catch (e: any) {
    logErrorWithLog(null, 'api_error', e.message, 'patch_device');
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
// ── Zigbee2MQTT helper — получает устройства через MQTT bridge ──
// HTTP API Z2M не используется (в Z2M v2.12.x frontend не включает API).
// Вместо этого получаем данные через WebSocket-клиент MQTT (mqtt-ws.ts).
const Z2M_URL = process.env.Z2M_URL || '';
const Z2M_TOKEN = process.env.Z2M_AUTH_TOKEN || '';

function zigbeeRequest(path: string): Promise<any> {
  // Если Z2M URL не задан — не используем HTTP API (он не включён)
  if (!Z2M_URL) return Promise.reject(new Error('Z2M_URL not configured'));
    ...
    const req = httpGet(`${Z2M_URL}${path}`, { headers, timeout: 5000 }, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          reject(new Error('Invalid JSON from Z2M'));
        }
      });
    });
    req.on('error', reject);
...
app.post('/api/discovery/:ieee/confirm', async (req, res) => {
  try {
    ...
    // Полный upsert room_id
    await query(`
      UPDATE devices 
      SET room_id = ?, 
          room_manually_set = 1 
      WHERE ieee_addr = ?
    `, roomId ? Number(roomId) : null, ieee);
...
// ── GET /api/house-mode ──────────────────────────────
app.get('/api/house-mode', (_req, res) => {
  res.json({ ok: true, mode: getCurrentHouseMode() });
});

// ── POST /api/house-mode ───────────────────────────────
app.post('/api/house-mode', commandLimiter, async (req, res) => {
  try {
    const { mode } = req.body;
    if (!mode) return res.status(400).json({ ok: false, error: 'mode is required' });
    const modes = await query('SELECT name FROM house_modes');
    const validModes = (modes as any[]).map((m: any) => m.name);
    if (!validModes.includes(mode)) {
      return res.status(400).json({ ok: false, error: `Invalid mode: ${mode}. Valid modes: ${validModes.join(', ')}` });
    }
...
// ── GET /api/house-modes ───────────────────────────────
app.get('/api/house-modes', async (_req, res) => {
  try {
    const rows = await query('SELECT * FROM house_modes ORDER BY name');
...
// ── GET /api/scenarios/blueprints (ДО параметризованных маршрутов :id) ─
app.get('/api/scenarios/blueprints', blueprintsHandler);
...
app.post('/api/scenarios', async (req, res) => {
  try {
    const {
      name, description, triggers_json: rawTriggersJson, actions_json,
      schedule_json: explicitScheduleJson, active, trigger_logic, conditions_json,
      priority_level, run_mode, enabled_house_modes, cooldown_ms, debounce_ms,
      timeout_sec, parent_id
    } = req.body;
    ...
    let triggers_json = split.triggers_json;
    ...
    // Блокируем пустые triggers
    if (parsedTriggers.length === 0) {
      return res.status(400).json({ ok: false, error: 'triggers_json: требуется минимум один триггер' });
    }
    // Валидация priority_level
    if (priority_level !== undefined) {
      const pl = Number(priority_level);
      if (!Number.isInteger(pl) || pl < 1 || pl > 5) {
        return res.status(400).json({ ok: false, error: 'priority_level: допустимые значения 1..5' });
      }
    }
    // Валидация run_mode
    const VALID_RUN_MODES = ['single', 'queued', 'restart', 'parallel'];
    if (run_mode !== undefined && !VALID_RUN_MODES.includes(run_mode)) {
      return res.status(400).json({ ok: false, error: `run_mode: допустимые значения: ${VALID_RUN_MODES.join(', ')}` });
    }
    // Валидация неотрицательных целых
    for (const [field, val] of Object.entries({ cooldown_ms, debounce_ms, timeout_sec })) {
      if (val !== undefined) {
        const n = Number(val);
        if (!Number.isInteger(n) || n < 0) {
          return res.status(400).json({ ok: false, error: `${field}: должно быть неотрицательным целым` });
        }
      }
    }
    // Генерация webhook_id для триггеров типа webhook
    const webhookTriggers = (parsedTriggers as any[]).filter((t: any) => t.type === 'webhook' && !t.webhook_id);
    if (webhookTriggers.length > 0) {
      const updated = (parsedTriggers as any[]).map((t: any) => {
        if (t.type === 'webhook' && !t.webhook_id) {
          return { ...t, webhook_id: randomBytes(16).toString('hex') };
        }
        return t;
      });
      triggers_json = JSON.stringify(updated);
    }
    ...
    await query(
      `INSERT INTO scenarios (id, name, description, triggers_json, actions_json, schedule_json, active,
        trigger_logic, conditions_json, priority_level, run_mode, enabled_house_modes,
        cooldown_ms, debounce_ms, timeout_sec, parent_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      id, name, description || null, triggers_json, actions_json,
      schedule_json || null,
      active !== undefined ? (active ? 1 : 0) : 1,
      trigger_logic || null, conditions_json || null,
      priority_level !== undefined ? Number(priority_level) : 3,
      run_mode || 'single',
      enabled_house_modes || JSON.stringify(['home', 'away', 'night', 'sleep', 'guest', 'vacation', 'work']),
      cooldown_ms !== undefined ? Number(cooldown_ms) : 2000,
      debounce_ms !== undefined ? Number(debounce_ms) : 0,
      timeout_sec !== undefined ? Number(timeout_sec) : 0,
      parent_id || null
    );
...
app.put('/api/scenarios/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const {
      name, description, triggers_json, actions_json, schedule_json, active,
      trigger_logic, conditions_json, priority_level, run_mode, enabled_house_modes,
      cooldown_ms, debounce_ms, timeout_sec, parent_id
    } = req.body;
    ...
    if (triggers_json !== undefined) {
      ...
      // Генерация webhook_id для триггеров типа webhook
      let finalTriggersJson = split.triggers_json;
      const webhookTriggers = (parsedTriggers as any[]).filter((t: any) => t.type === 'webhook' && !t.webhook_id);
      if (webhookTriggers.length > 0) {
        const updated = (parsedTriggers as any[]).map((t: any) => {
          if (t.type === 'webhook' && !t.webhook_id) {
        ...
        finalTriggersJson = JSON.stringify(updated);
      }
      updates.push('triggers_json = ?'); params.push(finalTriggersJson);
    ...
    if (trigger_logic !== undefined) { updates.push('trigger_logic = ?'); params.push(trigger_logic); }
    if (conditions_json !== undefined) { updates.push('conditions_json = ?'); params.push(conditions_json); }
    if (priority_level !== undefined) {
      const pl = Number(priority_level);
      if (!Number.isInteger(pl) || pl < 1 || pl > 5) {
        return res.status(400).json({ ok: false, error: 'priority_level: допустимые значения 1..5' });
      }
      updates.push('priority_level = ?'); params.push(pl);
    }
    if (run_mode !== undefined) {
      if (!['single', 'queued', 'restart', 'parallel'].includes(run_mode)) {
        return res.status(400).json({ ok: false, error: 'run_mode: допустимые значения: single, queued, restart, parallel' });
      }
      updates.push('run_mode = ?'); params.push(run_mode);
    }
    if (enabled_house_modes !== undefined) { updates.push('enabled_house_modes = ?'); params.push(enabled_house_modes); }
    if (cooldown_ms !== undefined) {
      const n = Number(cooldown_ms);
      if (!Number.isInteger(n) || n < 0) return res.status(400).json({ ok: false, error: 'cooldown_ms: должно быть неотрицательным целым' });
      updates.push('cooldown_ms = ?'); params.push(n);
    }
    if (debounce_ms !== undefined) {
      const n = Number(debounce_ms);
      if (!Number.isInteger(n) || n < 0) return res.status(400).json({ ok: false, error: 'debounce_ms: должно быть неотрицательным целым' });
      updates.push('debounce_ms = ?'); params.push(n);
    }
    if (timeout_sec !== undefined) {
      const n = Number(timeout_sec);
      if (!Number.isInteger(n) || n < 0) return res.status(400).json({ ok: false, error: 'timeout_sec: должно быть неотрицательным целым' });
      updates.push('timeout_sec = ?'); params.push(n);
    }
...
app.get('/api/scenarios/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const rows = await query('SELECT * FROM scenarios WHERE id = ?', id);
    if (!rows.length) return res.status(404).json({ ok: false, error: 'Scenario not found' });
    res.json({ ok: true, scenario: rows[0] });
  } catch (e: any) {
    logErrorWithLog(null, 'api_error', e.message, 'scenario_get');
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.post('/api/scenarios/:id/run-now', commandLimiter, async (req, res) => {
  try {
    const { id } = req.params;
    const rows = await query('SELECT * FROM scenarios WHERE id = ?', id);
    if (!rows.length) return res.status(404).json({ ok: false, error: 'Scenario not found' });
    ...
    if (!actions) return res.status(400).json({ ok: false, error: 'Invalid actions_json' });
    // Run-now bypasses triggers but not conditions
    const { evaluateConditions, getCurrentHouseMode } = await import('./engine');
    const condResult = evaluateConditions(scenario.conditions_json || null, new Map(), getCurrentHouseMode());
    if (!condResult) {
      return res.status(400).json({ ok: false, error: 'Conditions (guard) not met — run-now skipped' });
    }
    const { executeActions } = await import('./actions');
    const execResult = await executeActions(actions, scenario.name, scenario.timeout_sec || 0);
    const { logScenarioExec } = await import('./db');
    logScenarioExec(scenario.id, 'run-now', execResult.fired, execResult.errors.length === 0,
      execResult.errors.length > 0 ? execResult.errors.join('; ') : undefined);
    res.json({ ok: true, scenario_id: scenario.id, fired: execResult.fired, errors: execResult.errors });
  } catch (e: any) {
    logErrorWithLog(null, 'api_error', e.message, 'scenario_run_now');
    res.status(500).json({ ok: false, error: e.message });
  }
});

// ── POST /api/scenarios/webhook/:webhook_id ──────────────
// НЕ требует авторизации (внешние сервисы должны иметь возможность его вызвать)
app.post('/api/scenarios/webhook/:webhook_id', webhookLimiter, (req, res) => {
  try {
    const { webhook_id } = req.params;
    // Find all active scenarios with webhook trigger matching this webhook_id
    const scenarios = db.prepare(`SELECT id, name, triggers_json, actions_json, conditions_json, timeout_sec
      FROM scenarios WHERE active = true AND triggers_json LIKE '%"type":"webhook"%'`).all() as any[];
    const matched = scenarios.filter(s => {
      try {
        const t = JSON.parse(s.triggers_json);
        if (Array.isArray(t)) return t.some((tr: any) => tr.type === 'webhook' && tr.webhook_id === webhook_id);
        if (t.conditions) return t.conditions.some((c: any) => c.type === 'webhook' && c.webhook_id === webhook_id);
        return false;
      } catch { return false; }
    });
    if (matched.length === 0) return res.status(404).json({ ok: false, error: 'No webhook trigger found for this id' });
    ...
    for (const s of matched) {
      Promise.resolve().then(async () => {
        try {
          ...
          if (!actions) return;
          const { executeActions } = await import('./actions');
          const execResult = await executeActions(actions, s.name, s.timeout_sec || 0);
          const { logScenarioExec } = await import('./db');
          logScenarioExec(s.id, `webhook:${webhook_id}`, execResult.fired, execResult.errors.length === 0,
            execResult.errors.length > 0 ? execResult.errors.join('; ') : undefined);
        } catch (e: any) {
          logErrorWithLog(null, 'webhook_exec_error', e.message, s.name);
        }
      });
    }
    res.json({ ok: true, triggered: matched.length, scenario_names: matched.map((m: any) => m.name) });
  } catch (e: any) {
    logErrorWithLog(null, 'api_error', e.message, 'scenario_webhook');
    res.status(500).json({ ok: false, error: e.message });
  }
});

// ── GET /api/blueprints (и алиас для фронтенда) ──────────
app.get('/api/blueprints', blueprintsHandler);
// Алиас уже объявлен выше (перед GET /api/scenarios) — этот дубликат удалён

function blueprintsHandler(_req: any, res: any) {
  try {
    const fs = require('fs');
    const path = require('path');
    const dir = path.join(__dirname, '..', 'blueprints');
    if (!fs.existsSync(dir)) return res.json({ ok: true, blueprints: [] });
    const files = fs.readdirSync(dir).filter((f: string) => f.endsWith('.json'));
    const blueprints = files.map((f: string) => {
      try {
...
async function instantiateBlueprint(req: any, res: any) {
  try {
    const fs = require('fs');
    const path = require('path');
    const bpPath = path.join(__dirname, '..', 'blueprints', `${req.params.id}.json`);
    if (!fs.existsSync(bpPath)) return res.status(404).json({ ok: false, error: 'Blueprint not found' });
    const blueprint = JSON.parse(fs.readFileSync(bpPath, 'utf-8'));
    const params = req.body.params || {};

    // Auto-fill missing params: use default from blueprint or sensible defaults
    for (const p of (blueprint.required_params || [])) {
      if (params[p.key] === undefined || params[p.key] === '' || params[p.key] === null) {
        if (p.default !== undefined) {
          params[p.key] = p.default;
        } else if (p.type === 'room') {
          // Подставляем первую комнату из БД
          try {
            const rooms = await query('SELECT id, name FROM rooms WHERE id > 0 ORDER BY id LIMIT 1');
            if (rooms.length > 0) params[p.key] = rooms[0].id;
          } catch {}
        } else if (p.type === 'device') {
          // Для девайсов — пропускаем проверку, рантайм сам подберёт
          params[p.key] = '';
        }
      }
    }

    // Validate required params (только те что реально нужны и не автозаполнились)
    const missing = (blueprint.required_params || []).filter((p: any) => (params[p.key] === undefined || params[p.key] === '' || params[p.key] === null) && p.default === undefined);
    if (missing.length > 0) {
      return res.status(400).json({
        ok: false,
        error: `Missing required parameters: ${missing.map((p: any) => p.key).join(', ')}`
      });
    }

    // Substitute placeholders
    const substitute = (template: string, paramValues: Record<string, any>): string => {
      return template.replace(/\{\{param:(\w+)\}\}/g, (_: string, key: string) => {
        return String(paramValues[key] || '');
      });
    };

    const triggersJson = substitute(blueprint.triggers_json || '[]', { ...params, ...Object.fromEntries(
      (blueprint.required_params || []).map((p: any) => [p.key, params[p.key] ?? p.default])
    )});
    const actionsJson = substitute(blueprint.actions_json || '[]', { ...params, ...Object.fromEntries(
      (blueprint.required_params || []).map((p: any) => [p.key, params[p.key] ?? p.default])
    )});
    const scheduleJson = blueprint.schedule_json || null;
    const conditionsJson = blueprint.condition ? JSON.stringify([{ type: 'mode', house_mode: [params.mode || ''] }]) : null;

    // Parse and validate via POST /api/scenarios
    const { triggers_json: cleanTriggers, schedule_json: cleanSchedule, error } = splitScheduleFromTriggers(triggersJson);
    if (error) return res.status(400).json({ ok: false, error });

    const parsedTriggers = parseTriggers(cleanTriggers);
    const parsedActions = parseActions(actionsJson);
    if (!parsedTriggers || !parsedActions) {
      return res.status(400).json({ ok: false, error: 'Invalid trigger or action after substitution' });
    }

    const maxId = await query('SELECT COALESCE(MAX(id),0)+1 as next_id FROM scenarios');
    const id = maxId[0].next_id;
    const name = params.name || blueprint.name;

    await query(
      `INSERT INTO scenarios (id, name, triggers_json, actions_json, schedule_json, conditions_json, active)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      id, name, cleanTriggers, actionsJson, cleanSchedule || scheduleJson, conditionsJson, 1
    );
    ...
    const s = await query('SELECT * FROM scenarios WHERE id = ?', id);
    res.status(201).json({ ok: true, scenario: s[0], blueprint_id: req.params.id });
  } catch (e: any) {
    res.status(500).json({ ok: false, error: e.message });
  }
}

// ── Маршруты для instantiateBlueprint ────────────────
app.post('/api/blueprints/:id/instantiate', instantiateBlueprint);
app.post('/api/scenarios/blueprints/:name/create', (req: any, _res: any, next: any) => {
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!key || typeof key !== 'string') {
      return res.status(400).json({ ok: false, error: 'Требуется поле key' });
    }
    if (!validateApiKey(key)) {
      logSecurityEvent({ type: 'auth_failed', ip: req.ip || 'unknown', detail: 'login_attempt' });
      return res.status(401).json({ ok: false, error: 'Неверный ключ' });
    }
    logSecurityEvent({ type: 'auth_success', ip: req.ip || 'unknown', detail: 'login' });
    res.json({ ok: true, token: key });
  } catch (e: any) {
    logErrorWithLog(null, 'api_error', e.message, 'login');
