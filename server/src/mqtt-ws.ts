...
import { stmt, db, logErrorWithLog, logStateChange, query, DB_PATH, logScenarioExec } from './db';
import { validateMqttPayload, type MqttTelemetryPayload } from './schemas';
import { parseActions, executeActions } from './actions';
import { evaluateTelemetry } from './engine';
...
// Кеш friendly_name → ieee для fuzzy match (обновляется при bridge/devices)
const deviceNameCache = new Map<string, string>();

// WebSocket broadcast queue — защита от медленных клиентов
const wsQueue: string[] = [];
let wsDraining = false;
function drainWsQueue() {
  if (!wss || wsDraining) return;
  wsDraining = true;
  const clientList = [...wss.clients].filter((c: WSClient) => c.readyState === 1);
  if (clientList.length === 0) { wsDraining = false; return; }
  // Берём одно сообщение за раз, чтобы не блокировать event loop
  const msg = wsQueue.shift();
  if (!msg) { wsDraining = false; return; }
  let remaining = clientList.length;
  for (const c of clientList) {
    try {
      // Если клиент не успевает — пропускаем его, не ждём
      if (c.bufferedAmount > 65536) continue;
      c.send(msg);
    } catch {}
    remaining--;
  }
  if (remaining === 0 && wsQueue.length > 0) {
    setImmediate(drainWsQueue);
  } else {
    wsDraining = false;
  }
}
function broadcastWS(msg: string) {
  wsQueue.push(msg);
  if (!wsDraining) setImmediate(drainWsQueue);
}
...
export function connectMQTT() {
  ...
  client.on('connect', () => {
    ...
    // Запрашиваем список устройств у Z2M — после перезапуска сервера
    // Z2M поддерживает bridge/request/devices (возвращает список на zigbee2mqtt/bridge/devices)
    client!.publish('zigbee2mqtt/bridge/request/devices', '{"type":"devices"}');
    logger.log("[MQTT-WS] ", '📡 Запрошен список устройств от Z2M');
  });
...
// Дополнительные MQTT топики, на которые подписываются сценарии
let extraSubscriptions = new Set<string>();

export function subscribeScenarioTopic(topic: string): void {
  if (!extraSubscriptions.has(topic) && client) {
    extraSubscriptions.add(topic);
    client.subscribe(topic, (err) => {
      if (err) logErrorWithLog(null, 'mqtt_subscribe_error', err.message, topic);
      else logger.log("[MQTT-WS] ", `📡 Scenario subscribed: ${topic}`);
    });
  }
}

export function unsubscribeScenarioTopic(topic: string): void {
  extraSubscriptions.delete(topic);
  // Keep subscription if other scenarios still use it (check on reload)
  if (client) {
...
function handleMessage(topic: string, payload: Buffer) {
  ...
  // Check for scenario MQTT triggers on non-zigbee2mqtt topics
  if (topicParts[0] !== 'zigbee2mqtt') {
    // Check if any scenario has an MQTT trigger for this topic
    try {
      const payloadStr = payload.toString('utf8');
      // Find scenarios with matching MQTT triggers
      const scenarios = db.prepare(`SELECT id FROM scenarios WHERE active = true AND triggers_json LIKE '%"type":"mqtt"%'`).all() as any[];
      let fired = false;
      for (const s of scenarios) {
        try {
          const t = JSON.parse(s.triggers_json);
          const triggers = Array.isArray(t) ? t : (t.conditions || []);
          for (const tr of triggers) {
            if (tr.type === 'mqtt' && tr.topic === topic) {
              if (!tr.payload || tr.payload === payloadStr) {
                // Trigger matched — fire scenario
                const scenario = db.prepare(`SELECT * FROM scenarios WHERE id = ?`).get(s.id) as any;
                ...
                (async () => {
                  ...
                  logScenarioExec(scenario.id, `mqtt:${topic}`, execResult.fired,
  ...
  try {
    ...
    if (topicParts[1] === 'bridge') {
      ...
      // 🔥 Всегда логируем сырое bridge-событие для отладки discovery
      logger.log("[MQTT-WS] ", `🌉 Bridge: ${event} → ${JSON.stringify(data).slice(0, 500)}`);
...
function handleDeviceDiscovery(friendlyName: string, data: any) {
  ...
  try {
    ...
    // Всегда пробуем AI, если есть модель — AI может скорректировать ошибки классификатора
    if (model) {
      (async () => {
        try {
          ...
          if (aiType && aiType !== detectedType) {
            logger.log("[MQTT-WS] ", `🧠 AI скорректировал тип: ${detectedType || 'null'} → ${aiType} для ${name} (${model})`);
            try {
              db.prepare(`UPDATE discovery_events SET suggested_type = ? WHERE ieee_address = ? AND status = 'pending'`).run(aiType, ieee);
            } catch {}
          } else if (aiType) {
            logger.log("[MQTT-WS] ", `🧠 AI подтвердил тип: ${aiType} для ${name} (${model})`);
          }
        } catch {}
      })();
    }
...
function handleBridgeDevices(devices: any) {
  ...
  for (const dev of devices) {
    // ⚡ Router'ы тоже могут быть датчиками (Tuya TS0601 с mmWave, CO2 и т.д.)
    // Фильтруем только Coordinator и disabled, а Router остаётся если есть definition
    if (dev.type === 'Coordinator' || dev.disabled) continue;
    // Router без definition — не показываем (чистый ретранслятор)
    if (dev.type === 'Router' && !dev.definition?.model && !dev.model_id) continue;
    ...
    try {
      ...
      // ⚡️ Вместо upsertDeviceFromDiscovery (который добавляет прямо в devices) —
      // пишем в discovery_events, чтобы пользователь подтвердил вручную
      try {
        const existing = db.prepare(`SELECT ieee_addr FROM devices WHERE ieee_addr = ? LIMIT 1`).get(ieee);
        if (!existing) {
          stmt.addDiscoveryEvent.run(ieee, name, model, vendor, 'bridge_device', 'pending', detectedType, null);
          logger.log("[MQTT-WS] ", `📦 Bridge device (pending): ${name} (${model}) → ${detectedType}`);
        } else {
          // Уже добавлен вручную — просто обновляем
          stmt.upsertDeviceFromDiscovery.run(ieee, name, model, vendor, detectedType, null);
          logger.log("[MQTT-WS] ", `📦 Bridge device (known): ${name} (${model})`);
        }
        // Обновляем кеш friendly_name → ieee
        deviceNameCache.set(name.toLowerCase(), ieee);
      } catch {
        // Fallback — upsert напрямую
        stmt.upsertDeviceFromDiscovery.run(ieee, name, model, vendor, detectedType, null);
...
export function mapZ2MTypeToInternal(ieeeAddr: string, exposes: any[] | null, model?: string | null, vendor?: string | null): string | null {
  ...
  // 1. Если устройство УЖЕ в БД (devices) — используем его тип, не трогаем discovery_events
  try {
    const existing = db.prepare(
      `SELECT type FROM devices WHERE ieee_addr = ? LIMIT 1`
    ).get(ieeeAddr) as { type: string } | undefined;
    if (existing?.type) {
      return existing.type;
    }
  } catch {}

  // 2. Для НОВЫХ устройств (нет в БД) — проверяем suggested_type из discovery_events
  try {
  ...
  // Remote/button — батарейка + action (single/double/hold) = пульт ДУ
  if (features.has('action') && features.has('battery')) return 'remote';

  if (types.has('switch')) {
    // Если есть power/energy/current — это розетка (plug)
    if (features.has('power') || features.has('energy') || features.has('current') || features.has('ratedPower')) return 'plug';
    // Если есть батарейка и нет power — это скорее remote (не путаем с plug)
    if (features.has('battery')) return 'remote';
    return 'plug';
  }
...
// Асинхронная обёртка: синхронный profile + exposes, затем AI.
// AI всегда вызывается для уточнения (даже если syncType есть).
export async function detectDeviceTypeFull(
  ...
  // 2. Если нет модели — возвращаем syncType (AI без модели бесполезен)
  if (!model) return syncType;

  // 3. Всегда пробуем AI — может скорректировать ошибки классификатора
  try {
    const { detectDeviceTypeWithAI } = await import('./ai');
    ...
    if (aiType) {
      logger.log("[MQTT-WS] ", `🧠 AI detection: ${model} → ${aiType}${syncType && aiType !== syncType ? ` (скорректировал ${syncType})` : ''}`);
...
// ── Device type detection from REAL MQTT data (not exposes) ──
// Zigbee2MQTT публикует реальные данные с датчика даже если не знает модель.
// Этот детектор анализирует поля payload и определяет тип по наличию характерных полей.
function detectDeviceTypeFromData(data: any): string | null {
  if (data.co2 !== undefined || data.voc !== undefined ||
      data.formaldehyde !== undefined || data.pm25 !== undefined ||
      data.pm10 !== undefined || data.air_quality !== undefined) {
    return 'air_monitor';
  }
  if (data.presence === true || data.presence === 1 || data.motion === true || data.motion === 1) {
  ...
  if (data.contact !== undefined) {
    return data.tamper !== undefined ? 'window_sensor' : 'door_sensor';
  }
  ...
  if (data.current !== undefined || data.power !== undefined || data.energy !== undefined) {
    return 'plug';
  }
  if (data.temperature !== undefined && data.humidity !== undefined) {
    // Если есть co2/voc — уже выше вернули air_monitor. Здесь только temp+humidity.
    return 'temp_sensor';
  }
  if (data.temperature !== undefined && data.humidity === undefined) {
    return 'temp_sensor';
...
function handleTelemetry(friendlyName: string, data: any) {
  ...
  // If telemetry has no ieee_address, look it up from cache first, then DB
  if (!ieee) {
    // Сначала проверяем кеш
    const cached = deviceNameCache.get(friendlyName.toLowerCase());
    if (cached) {
      ieee = cached;
    } else {
      try {
        const row = db.prepare(
          `SELECT ieee_addr, friendly_name FROM devices WHERE friendly_name LIKE ? LIMIT 1`
        ).get(`%${friendlyName.replace(/'/g, "''")}%`) as any;
        if (row?.ieee_addr && /^0x[0-9a-f]{16}$/i.test(row.ieee_addr)) {
          ieee = row.ieee_addr;
          deviceNameCache.set(friendlyName.toLowerCase(), ieee);
          logger.log("[MQTT-WS] ", `🔍 Cached ${friendlyName} → ${ieee}`);
        } else {
          logger.log("[MQTT-WS] ", `⏭️ Skipping telemetry from ${friendlyName}: no ieee_address and device not registered`);
          return;
        }
      } catch (e: any) {
        logger.log("[MQTT-WS] ", `⏭️ Skipping telemetry from ${friendlyName}: DB lookup failed (${e.message})`);
        return;
      }
    }
  }
  ...
  if (stored > 0) {
    ...
    // ── Data-based device type re-detection ──
    // После получения реальных данных с датчика проверяем, соответствует ли
    // текущий тип тому, что говорят данные. Если нет — исправляем.
    // Это решает проблему, когда Z2M не знает модель (exposes пустые) и AI гадает.
    try {
      const dataType = detectDeviceTypeFromData(data);
      if (dataType) {
        const currentType = db.prepare(
          "SELECT type, type_manually_set FROM devices WHERE ieee_addr = ? LIMIT 1"
        ).get(ieee) as { type: string; type_manually_set: number } | undefined;
        if (currentType && currentType.type !== dataType && !currentType.type_manually_set) {
          logger.log("[MQTT-WS] ", `🧩 Data-based type correction: ${friendlyName} ${currentType.type} → ${dataType}`);
          db.prepare("UPDATE devices SET type = ? WHERE ieee_addr = ?").run(dataType, ieee);
          // Сохраняем в device_profiles, чтобы в следующий раз не переопределять
          try {
            const row = db.prepare(
              "SELECT model, vendor FROM devices WHERE ieee_addr = ?"
            ).get(ieee) as { model: string | null; vendor: string | null } | undefined;
            if (row?.model) {
              stmt.saveDeviceProfile.run(row.model, row.vendor || "", null, dataType, null, null, null, null, null, null, null);
            }
          } catch {}
        }
      }
    } catch (e: any) {
      logErrorWithLog(ieee, "data_detect_error", e.message, friendlyName);
    }
