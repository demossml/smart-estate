...
// ⚠️ ОСТОРОЖНО: реальные комнаты пользователя имеют ID 2-5
// Закомментировано — восстановлено через ручную вставку
// const legacyRoomCount = db.prepare(...)
...
// ── Migration: Scenario System (Фаза 0, промпт 0.1) ──────────────────
// Создаём таблицы manual_overrides и house_modes
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_override_device ON manual_overrides(device_ieee);

  CREATE TABLE IF NOT EXISTS house_modes (
    name            TEXT PRIMARY KEY,
    display_name    TEXT NOT NULL,
    default_active  INTEGER DEFAULT 1,
    icon            TEXT
  );
  INSERT OR IGNORE INTO house_modes VALUES ('home',     'Дома',         1, 'home');
  INSERT OR IGNORE INTO house_modes VALUES ('away',     'Отсутствует',  1, 'away');
  INSERT OR IGNORE INTO house_modes VALUES ('night',    'Ночь',         1, 'night');
  INSERT OR IGNORE INTO house_modes VALUES ('sleep',    'Сон',          1, 'sleep');
  INSERT OR IGNORE INTO house_modes VALUES ('guest',    'Гости',        1, 'guest');
  INSERT OR IGNORE INTO house_modes VALUES ('vacation', 'Отпуск',       1, 'vacation');
  INSERT OR IGNORE INTO house_modes VALUES ('work',     'Работаю дома', 1, 'work');
`);

// Добавляем новые колонки в scenarios (идемпотентно через ALTER TABLE)
const scenarioCols = [
  'room_id INTEGER',
  'trigger_logic TEXT DEFAULT \'ANY\'',
  'conditions_json TEXT',
  'priority_level INTEGER DEFAULT 3',
  'run_mode TEXT DEFAULT \'single\'',
  `enabled_house_modes TEXT DEFAULT '["home","away","night","sleep","guest","vacation","work"]'`,
...
for (const colDef of scenarioCols) {
  const colName = colDef.split(' ')[0];
  const exists = (db.prepare(`SELECT COUNT(*) as cnt FROM pragma_table_info('scenarios') WHERE name = ?`).get(colName) as any).cnt;
  if (!exists) {
    db.exec(`ALTER TABLE scenarios ADD COLUMN ${colDef}`);
    logger.log("[DB] ", `➕ Сценарии: колонка ${colName} добавлена`);
  }
}

// Индексы для scenario_executions
try { db.exec(`CREATE INDEX IF NOT EXISTS idx_exec_scenario ON scenario_executions(scenario_id, ts DESC)`); } catch {}
try { db.exec(`CREATE INDEX IF NOT EXISTS idx_exec_ts ON scenario_executions(ts DESC)`); } catch {}

// Преобразование старого формата triggers_json в новый (массив)
// Старый формат: {"logic":"ANY","conditions":[{"device":"...","property":"...","operator":"=","value":1}]}
// Новый формат: [{"type":"device","device":"...","property":"...","operator":"=","value":1}]
const oldScenarios = db.prepare(`SELECT id, triggers_json, schedule_json FROM scenarios WHERE triggers_json LIKE '{%' AND id NOT IN (SELECT id FROM scenarios WHERE triggers_json LIKE '[%')`).all() as any[];
for (const s of oldScenarios) {
  try {
    const oldTrig = JSON.parse(s.triggers_json);
    if (oldTrig && oldTrig.conditions && Array.isArray(oldTrig.conditions)) {
      const newTriggers = oldTrig.conditions.map((c: any) => ({
        type: 'device',
        device: c.device,
        property: c.property,
        operator: c.operator,
      ...
      const newTriggersJson = JSON.stringify(newTriggers);
      db.prepare(`UPDATE scenarios SET trigger_logic = ?, triggers_json = ? WHERE id = ?`).run(
        oldTrig.logic || 'ANY', newTriggersJson, s.id
      );
      logger.log("[DB] ", `🔄 Сценарий #${s.id}: triggers_json конвертирован в новый формат`);
    }
  } catch {}
}

// Преобразование старого формата actions_json (обёртывание в delay-структуру, если нужно)
// Старый: [{"type":"mqtt","device":"...","command":"ON"},{"type":"notify","message":"..."}]
// Новый: должен быть массивом — он уже массив, но типы могут быть старые
// Заменяем type:"mqtt" на type:"device_command" — унификация
const allScenarios = db.prepare(`SELECT id, actions_json FROM scenarios`).all() as any[];
for (const s of allScenarios) {
  try {
    const acts = JSON.parse(s.actions_json);
    if (Array.isArray(acts)) {
      ...
      if (changed) {
        db.prepare(`UPDATE scenarios SET actions_json = ? WHERE id = ?`).run(JSON.stringify(acts), s.id);
        logger.log("[DB] ", `🔄 Сценарий #${s.id}: actions_json type:'mqtt' → type:'device_command'`);
      }
    }
  } catch {}
}
...
export const stmt: any = {
  ...
  insertScenario: db.prepare(`INSERT INTO scenarios (id, name, description, room_id, triggers_json, trigger_logic, conditions_json, actions_json, schedule_json, priority_level, run_mode, active, enabled_house_modes, cooldown_ms, debounce_ms, timeout_sec, parent_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),
  updateScenario: db.prepare(`UPDATE scenarios SET name = ?, description = ?, room_id = ?, triggers_json = ?, trigger_logic = ?, conditions_json = ?, actions_json = ?, schedule_json = ?, priority_level = ?, run_mode = ?, active = ?, enabled_house_modes = ?, cooldown_ms = ?, debounce_ms = ?, timeout_sec = ? WHERE id = ?`),
  deleteScenarioExecutions: db.prepare(`DELETE FROM scenario_executions WHERE scenario_id = ?`),
